...
#####################################
set output 'B3sphere_dif_mp_splitcase_all_more.eps'
...
set title font "times,14" '1 m \& 1.5 el/m' 
plot loc.'1_above/measurements_spiral.ascii' u 0:(($10-$7)*1e6) w p ls 4 t '{Bx_{a}-Bx_{c}',\
     loc.'1_above/measurements_spiral.ascii' u 0:(($11-$8)*1e6) w p ls 3 t '{By_{a}-By_{c}',\
     loc.'1_above/measurements_spiral.ascii' u 0:(($12-$9)*1e6) w p ls 5 t '{Bz_{a}-Bz_{c}'
...
loc = './'
set terminal pngcairo enhanced size 1800,600
...
#####################################
set output 'B3sphere_dif_mp_splitcase_all_more.png'
set multiplot layout 1,5
set yrange [-5:4]
set ytics -5,1,4
set ylabel font "times,14" '{/Symbol D}B_i [({/Symbol m}T]'
