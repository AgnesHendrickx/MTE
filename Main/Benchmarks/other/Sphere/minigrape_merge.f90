...
sphere=.true.    !3D sphere -> numerical solve of function if the M is only in z direction and angles of r 
line_vert=.false.    ! vertical -> numerical solve for dipole -> for far away
add_random=.false.     ! adding random noise in the z direction 

!----------------------------
!options for Mag & B ref
!--------------------------------------
etna=.false.      !The IGRF & Mag, shape of sin of Etna, chosing a site results in flanks chosen 
...
Dres=2                   ! 2x2  or 5x5 m 
sDEM1=2                  ! to do domain size test, 1 only 2m DEM => site 1: 2000, 2: 1400, 3: 1000 4: 600 5:400  6:100 7:4000 8:6000
...
obs=3                    !vertical path is only used for a benchmark 

flank=0                 !Flank simulation Etna, 1=S, 2=E, 3=N, 4=W
!add IGRF/Bref, DEM site 1-6. If not chosen: IGRF average (south flank added if flanksim)
site=4                   !chosing a site overrules flank input if flanksim is true 
path=3
...
if (iproc==0) then
 ...
 if (.not. sphere .and. .not. use_path) then 
   ...
   if (.not. line_vert) print *,'height above=',h
  print *,'xmin=',xmin
  print *,'xmax=',xmax
  print *,'ymin=',ymin
  print *,'ymax=',ymax
...
if (bdeg>90 .or. bdeg<=-90) then 
 stop 'bdeg can not be outside of range <-90:90]'
end if 
if (only_top .and. .not. (add_sine .or. use_DEM .or. use_DEMPath))  then
stop 'can not do only top without adding topography'
...
elseif (Iter <=2 .and. use_DEMPath .and. Iter/=1) then
  stop 'Iteration of the path can not be 2, either >=3 or 1'
...
if (flanksim .and. .not. etna) then 
        stop 'flanksim is hardwired for etna' 
end if

if (flanksim .and. use_DEM) then
        stop 'either flanksim or DEM'
end if 
if (.not. add_sine) then 
  if (.not. use_meas .and. .not. use_path) then
        stop 'must choose computing observation points' 
end if 
end if
if (obs==2) then
  if (add_sine .or. use_DEM .or. only_top .or. add_random .or. flanksim .or. use_DEMPath .or. .not. line_vert) &
          stop 'observation vertical line only used for benchmark' 
end if 

!!site 4, aantal files hebben 000z in hoogtemeting field
! path=1 ho=1 
! path=2 ho=1

if (site==4) then
   if (path==1 .and. sDEM1==3) then
        stop 'Path 1 fals outside this DEM use cut of 300x300' 
  end if 
  if (path==1 .and. ho==1) then
        stop 'Field measurements of height 1 of path 1 at site 4 excluded' 
  end if 
  if (path==2 .and. ho==1) then
        stop 'Field measurements of height 1 of path 2 at site 4 excluded' 
  end if 
end if
...
if (use_DEM) then
 if (site==1) then
  ...
  if (Dres==2 .and. sDEM1==1) print*, 'reading from 2x2 DEM site 1 regular size (2000)'
  ...
  if (Dres==2 .and. sDEM1==8)  open(unit=123,file='DEMS/dem2m_site1_6000.asc', action='read')
  if (Dres==2 .and. sDEM1==8) print*, 'reading from 2x2 DEM site 1 6000 around paths'
 ...
 elseif (site==4 .or. site==6) then
  ...
  if (Dres==5 .and. site==4 .and. sDEM1==1)  print*, 'reading from 5x5 DEM site 4 and 6 2100'
  ...
  if (Dres==5 .and. site==6) then
      ...
      if (sDEM1==1)  print*, 'reading from 5x5 DEM site 6 2100'
    ...
    do j=0,nely 
     ...
     counter=counter+1
      if (only_top .and. k==nelz) then 
       ...
       if (iproc==0) then 
        if (obs==3 .and. j==nint(nely*0.5d0)) then
            write(321,*) x(counter),y(counter),z(counter)  !topography
        elseif (obs/=3) then  
...
!NO, ALL NEEDS TO BE? RIGHT? 
 ...
 !-----------------------------------------------------------------
   ...
   ! write(123,*) x0,(x0+xllcorner),y0,(y0+yllcorner),z0,Bx(i),By(i),Bz(i)
    write(123,*) x0,y0,z0,Bx(i),By(i),Bz(i)
