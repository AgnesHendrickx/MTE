...
## ONLY BENCHMARK=-1 (ETNA) & BENCHMARK=5 (FLANKSIM) ##
flat_bottom=False 
...
if benchmark=='4':
   ...
   yend=24.999999
   zend=0.00001
   #xstart=6.000000000001
   #ystart=-24.999999999999
   #zstart=0.0000003      
   #xend=6.000000000001
   #yend=24.999999999999
   #zend=0.0000003
   line_nmeas=21
   sphere_R=0
   sphere_xc=0
...
if benchmark=='-1' or (benchmark=='2a' and do_path_measurements):
...
if benchmark=='-1' or benchmark=='5':
    if flat_bottom and benchmark=='5':
       print('Bottom surface of domain chosen: Flat bottom, Lz added underneath path')
    elif flat_bottom and benchmark=='-1':
       print('Bottom surface of domain chosen: Flat bottom, Lz added to lowest point of DEM')
    else:
       print('Bottom surface of domain chosen: Repeat topography of the top')
...
if benchmark=='-1' and remove_box:
   xb = np.empty(NV,dtype=np.float64)  # x coordinates
   yb = np.empty(NV,dtype=np.float64)  # y coordinates
   zb = np.empty(NV,dtype=np.float64)  # z coordinates
...
###################################################################################################
# loading data from Ren (2017) for benchmark 4
if benchmark=='4':
   ...
   BxB4 = np.empty(line_nmeas,dtype=np.float64)  # Bx from Ren
   ByB4 = np.empty(line_nmeas,dtype=np.float64)  # By from Ren
   BzB4 = np.empty(line_nmeas,dtype=np.float64)  # Bz from Ren
...
if benchmark=='-1':
   ...
   if remove_box:
      xbpath = np.empty(npath,dtype=np.float64)  # x coordinates path box
      ybpath = np.empty(npath,dtype=np.float64)  # y coordinates
      zbpath = np.empty(npath,dtype=np.float64)  # z coordinates
...
export_mesh_3D(NV,nel,xb,yb,zb,icon,'mesh_box.vtu',Mx,My,Mz,nnx,nny,nnz)
...
#   linefile.write("# x,y,z, Bx_vi, By_vi, Bz_vi, Bx_si, By_si, Bz_si, In_si, Ic_si, Dc_si \n")
   linefile=open("measurements_line.ascii","w")
...
#       B_th[:,i]=compute_analytical_solution(xm,ym,zm,sphere_R,Mx0,My0,Mz0,sphere_xc,sphere_yc,sphere_zc,benchmark)
       ...
       if benchmark=='5':
          ...
          Ic_si[i]=np.arctan2(-B_si[2,i],np.sqrt(B_si[1,i]**2+B_si[0,i]**2))/np.pi*180
          Dc_si[i]=np.arctan2(B_si[0,i],B_si[1,i])/np.pi*180  
          #linefile.write("%.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f \n" %(xm,ym,zm,\
          #                                                                            B_si[0,i],B_si[1,i],B_si[2,i],\
          #                                                                            In_si[i],Ic_si[i],Dc_si[i]))
          linefile.write("%.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f \n" %(xm,ym,zm,\
                                                                                                        B_vi[0,i],B_vi[1,i],B_vi[2,i],\
                                                                                                        B_si[0,i],B_si[1,i],B_si[2,i],\
                                                                                                        In_si[i],Ic_si[i],Dc_si[i]))
          
       elif benchmark=='4':
          print('Obs point number=',(i+1), "Bx=",B_si[0,i], "By=",B_si[1,i], "Bz=",B_si[2,i]) 
          linefile.write("%.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f %.10f \n" %(xm,ym,zm,\
                                                           B_si[0,i]*1e9,B_si[1,i]*1e9,B_si[2,i]*1e9,\
                                                           BxB4[i],ByB4[i],BzB4[i]))
       else:
...
if do_path_measurements:
   ...
   if remove_box:
      print('starting path measurement for Box ...')

      linefile2=open("measurements_path_box.ascii","w")
      ...
      start = time.time()
      for i in range(0,npath):
          print('Box: doing',(i+1),'out of ',npath) 
          xm=xbpath[i] #xmeas
          ym=ybpath[i] #ymeas
          zm=zbpath[i] #zmeas
          #print(xm,ym,zm)
          ...
          if i==0:
             ...
             dmeas[i]=np.sqrt((xbpath[i]-xbpath[i-1])**2+(ybpath[i]-ybpath[i-1])**2)+dmeas[i-1]
          linefile2.write("%e %e %e %e %e %e %e %e %e %e \n" %(xm,ym,zm,\
                                                               Bb_si[1,i],Bb_si[0,i],-Bb_si[2,i],\
                                                               In_si[i],Ic_si[i],Dc_si[i],\
                                                               dmeas[i] ))    #AEH
      print("surf box int: %.3f s" % (time.time() - start))   
      export_line_measurements(npath,xbpath,ybpath,zbpath,'path_measurements_box.vtu',Bb_si,Bb_si,Bb_si)    #did not want to change tool, so useless 3x B_si
      print('========================================')
   start = time.time()
   for i in range(0,npath):
