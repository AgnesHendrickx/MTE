...
def add_referencefield(B0_name,npath,B0,B_si):
    """
    Returns total computed magnetic field, intentsity, inclination and declination
    at all points on a path by 
  
    1. rotating vector from model to paleomagnetism coordinate sytem,
   ...
   #    :return: 
   #     - B_siB0 (:py:class:`array_like(float)`) - 2D array(3,npath), containing components (0=x;1=y;2=z,npath) of the total magnetic field (anomalous + reference field) at each point of the path.
   #     - In_siB0 (:py:class:`array_like(float)`) - 1D array(npath), containing intensity of the total magnetic field (anomalous + reference field) at each point of the path.
   #     - Ic_siB0 (:py:class:`array_like(float)`) - 1D array(npath), containing inclination of the total magnetic field (anomalous + reference field) at each point of the path.
   #     - Dc_siB0 (:py:class:`array_like(float)`) - 1D array(npath), containing declination of total magnetic field (anomalous + reference field) at each point of the path.
   # :rtype: array(3,npath), array(npath), array(npath),array(npath)
...
def topography(x,y,A,llambda,cos_dir,sin_dir,slopex,slopey):
    ...
    :param cos_dir: cosine of ``direction``, that defines the direction of the wave function along the surface (default = for south/north it runs perpendicular to slope, for west/east parallel) 
    :type cos_dir: scalar(float)
    :param sin_dir: sine of ``direction``, that defines the direction of the wave function along the surface (for south/north it runs perpendicular to slope, for west/east parallel) 
    :type sin_dir: scalar(float)
    :param slopex: the arctan of the angle (in rad) of the surface that defines the slope in x direction  for each respective side **TODO: CHECK ARCTAN?**, used to compute added height due to slope of the flank on each side.
    :type slopex: scalar(float)
    :param slopey: the arctan of the angle (in rad) of the surface that defines the slope in y direction for each respective side **TODO: CHECK ARCTAN?**, used to compute added height due to slope of the flank on each side.
    :type slopey: scalar(float)

    :return: the height value for the point passed to function for the respective position on the flank (chosen by ``subbench``)
    :rtype: scalar(float)
...
def compute_analytical_solution(x,y,z,R,Mx,My,Mz,xcenter,ycenter,zcenter,benchmark):
   ...
   :param y: y coordinate of computation point 
   :type y: scalar(float) 
   :param z: z coordinate of computation point 
   :type z: scalar(float) 
   ...
   :return: 1D array containing components (0=x;1=y;2=z) produced by the analytical solution for the magnetic field for chosen benchmark (and model setup)  
...
if benchmark=='-1':
 ...
 # x[:]+=xllcorner #(centering)
 # y[:]+=yllcorner #(centering)
   x[:]+=xllcorner
   y[:]+=yllcorner
